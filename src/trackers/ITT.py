...
_itt_session_data: dict[str, dict[str, Union[str, None]]] = {}


class ITT(UNIT3D):
    """ItaTorrents tracker implementation with Italian localization support"""
    ...
    async def get_name(self, meta: dict[str, Any]) -> dict[str, str]:
        """
        Rebuild release name from meta components following ItaTorrents naming rules.
        ...
        name = None
        # Build name per ItaTorrents type-specific format
        if effective_type == "DISC":
            # Inject region from validated session data if available
            region = _itt_session_data.get(meta["uuid"], {}).get(
                "_itt_region_name"
            ) or meta.get("region", "")
            ...
            name = f"{title} {year} {season}{episode} {episode_title} {part} {three_d} {edition} {hybrid} {repack} {resolution} {uhd} {source} REMUX VU {hdr} {video_codec} {audio_lang_str} {audio}"
            ...
            name = f"{title} {year} {season}{episode} {episode_title} {part} {edition} {hybrid} {repack} {resolution} {type_str} {hdr} {video_codec} {audio_lang_str} {audio}"
            ...
            name = f"{title} {year} {season}{episode} {episode_title} {part} {edition} {hybrid} {repack} {resolution} {hdr} {video_codec} {audio_lang_str} {audio}"
    ...
    async def get_type_id(
        ...
        """Map release type to ItaTorrents type IDs"""
        type_mapping = {
            "DISC": "1",
            "REMUX": "2",
            "WEBDL": "4",
    ...
    async def get_additional_checks(self, meta: dict[str, Any]) -> Literal[True]:
        ...
        if meta.get("is_disc") in ["DVD", "HDDVD"]:
            ...
            if not region_name and (not meta.get("unattended") or meta.get("unattended_confirm")):
                while True:
                    region_name = cli_ui.ask_string(
                        "ITT: Region code not found for disc. Please enter it manually (mandatory): "
                    )
                    region_name = (
            ...
            if not region_name:
                cli_ui.error("Region required; skipping ITT.")
            ...
            if not region_id:
                cli_ui.error(f"Invalid region code '{region_name}'; skipping ITT.")
            ...
            if not distributor_name and not meta.get("unattended"):
                distributor_name = cli_ui.ask_string(
                    "ITT: Distributor (optional, Enter to skip): "
                )
                distributor_name = (
            ...
            _itt_session_data[meta["uuid"]] = {
                "_itt_region_id": region_id,
                "_itt_region_name": region_name,
                "_itt_distributor_id": distributor_id if distributor_name else None,
            }
    ...
    async def get_region_id(self, meta: dict[str, Any]) -> dict[str, Any]:
        ...
        data = _itt_session_data.get(meta["uuid"], {})
        region_id = data.get("_itt_region_id")
        if region_id:
    ...
    async def get_distributor_id(self, meta: dict[str, Any]) -> dict[str, Any]:
        ...
        data = _itt_session_data.get(meta["uuid"], {})
        distributor_id = data.get("_itt_distributor_id")
        if distributor_id:
    ...
    def _build_bbcode(
        ...
        """Build ItaTorrents BBCode template"""
        if category == "TV":
